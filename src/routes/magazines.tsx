...
const magazinesSearchSchema = z.object({
  tab: z.enum(["bulletin", "eastafrica", "business", "lifestyle", "technology", "archive"]).optional().catch("bulletin"),
...
export const Route = createFileRoute("/magazines")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Stay updated with local news bulletins and regional magazines in East Africa." },
    ],
  }),
...
const categories = [
  { id: "bulletin", label: "Breaking bulletin", desc: "Latest headlines from Kenya and East Africa" },
  ...
  { id: "lifestyle", label: "Lifestyle", desc: "Culture, entertainment and opinions" },
  { id: "technology", label: "Tech & Science", desc: "Innovation, research and digital future" },
  { id: "archive", label: "Internet Archive", desc: "Search digitised texts and historical periodicals" },
] as const;

function MagazinesPage() {
  const { tab: activeTab = "bulletin", q: searchQ = "" } = Route.useSearch();
  const navigate = useNavigate({ from: Route.fullPath });

  const [searchVal, setSearchVal] = useState(searchQ);
  const [debouncedQuery, setDebouncedQuery] = useState(searchQ);

  useEffect(() => {
    setSearchVal(searchQ);
  }, [searchQ]);

  // Handle Archive tab search debounce
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      navigate({
        search: (prev) => ({ ...prev, q: searchVal.trim() }),
        replace: true,
      });
    }, 450);
  ...
  const handleTabChange = (tabId: MagazinesTab) => {
    navigate({
      search: (prev) => ({ ...prev, tab: tabId, q: tabId === "archive" ? searchVal : "" }),
    });
  };
    ...
    <AppShell>
      <div className="px-5 pt-4">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Curated publications and local columns.
        </p>
        ...
        {/* Tab Description & Archive Search */}
          ...
          {activeTab === "archive" && (
            <div className="relative max-w-sm w-full">
              ...
              <input
                ...
                placeholder="Search Archive (e.g. Nairobi)"
                aria-label="Search internet archive texts"
                className="w-full pl-9 pr-4 py-2 rounded-md bg-muted text-xs placeholder:text-muted-foreground outline-none focus:ring-1 focus:ring-gold"
              />
            </div>
          )}
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {activeTab === "archive" && debouncedQuery.length <= 1
                  ? "Type at least 2 characters to search the Internet Archive."
